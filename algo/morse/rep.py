...
with open(file, 'r') as f:
    fichier = f.read()
    s = ""
    for i in range(1, len(fichier), 6):
        if fichier[i:i+6] in alp:
            s += alp[fichier[i:i+6]]

j = 2
for i in range(len(s)):
    test = lettre_d_affilee(s[i:], j)
    if test == True:
        print(s[i])
        j += 1
